...
function spinBtnHandler(time, interval) {
    ...
    let colourIndex = Math.floor(Math.random() * 4);
    selectedColour = coloursArray[colourIndex];
    // TODO randomly select bodyPart from array
    let bodyPartIndex = Math.floor(Math.random() * 4);
...
function addToHistory() {
    spinHistoryArray.push(new SpinRecord(selectedColour, selectedBodyPart, spinCount));
    ...
    document.getElementById('historyTableBody').innerHTML = historyTableBody;
}
function statsBtnHandler() {
    ...
    getAmountCounter = 0;
    let statsResults = '';
    let colourSelectorValue = document.getElementById('colourSelect').value;
    let bodyPartSelectorValue = document.getElementById('bodyPartSelect').value;
    ...
    document.getElementById('statsResults').innerHTML = statsResults;
}
